"""
=============================================================================
  LTCUSDT Cloud Feeder — Render.com version
  Runs 24/7 on Render free plan — no CPU limits
=============================================================================
"""
...
# ─────────────────────────────────────────────────────────────────────────────
#  CONFIGURATION — Render uses environment variables for paths
# ─────────────────────────────────────────────────────────────────────────────

# On Render, files are stored in /opt/render/project/src/
BASE_DIR   = Path("/opt/render/project/src")
TICKS_FILE = BASE_DIR / "ticks.csv"
HST_FILE   = BASE_DIR / "LTCUSDT1.hst"
HTML_OUT   = BASE_DIR / "static" / "index.html"
LOG_FILE   = BASE_DIR / "feeder.log"

# Rebuild chart every N ticks
REBUILD_EVERY = 500
...
def append_tick(ts_sec: int, price: float):
...
def rebuild_hst():
    log.info("Rebuilding LTCUSDT1.hst...")
    if not TICKS_FILE.exists():
        return
    ...
    with open(HST_FILE, 'wb') as fh:
        fh.write(struct.pack('<i', HST_VER))
        fh.write(b'\x00' * 64)
        fh.write(SYMBOL.encode('ascii')[:12].ljust(12, b'\x00'))
        fh.write(struct.pack('<i', 1))      # M1
    ...
    log.info(f"  HST: {len(ticks):,} bars  ({HST_FILE.stat().st_size/1024/1024:.1f} MB)")

# ─────────────────────────────────────────────────────────────────────────────
#  CASCADE + HTML BUILDER (inline — no external script needed)
# ─────────────────────────────────────────────────────────────────────────────
...
def run_cascade():
    """Read HST, run 24-stage cascade, return (m25_bricks, m31_bricks)."""
    ...
    with open(HST_FILE, 'rb') as f:
        f.seek(4+64+12+4+4+4+4+52)  # skip header
        while True:
            data = f.read(REC)
...
def rebuild_chart():
    """Run cascade and write HTML."""
    log.info("Running cascade and building HTML chart...")
    t0 = datetime.datetime.now()

    m25_bricks, m31_bricks = run_cascade()
    if not m25_bricks or not m31_bricks:
        log.warning("  No bricks produced — skipping HTML build")
        return

    elapsed = (datetime.datetime.now() - t0).total_seconds()
    log.info(f"  Cascade: M25={len(m25_bricks):,} M31={len(m31_bricks):,} ({elapsed:.1f}s)")

    # Export bricks as compact JSON
    m25_js = json.dumps(
        [[int(b['t']),round(b['o'],5),round(b['c'],5),int(b['v']),1 if b['bull'] else 0]
         for b in m25_bricks], separators=(',',':'))
    m31_js = json.dumps(
        [[int(b['t']),round(b['o'],5),round(b['c'],5),int(b['v']),1 if b['bull'] else 0]
         for b in m31_bricks], separators=(',',':'))

    # Get metadata
    first_dt = datetime.datetime.utcfromtimestamp(m25_bricks[0]['t']).strftime('%Y-%m-%d')
    last_dt  = datetime.datetime.utcfromtimestamp(m25_bricks[-1]['t']).strftime('%Y-%m-%d')
    gen_at   = datetime.datetime.utcnow().strftime('%Y-%m-%d %H:%M UTC')
    total_ticks = count_ticks()

    html = build_html(m25_js, m31_js,
                      len(m25_bricks), len(m31_bricks),
                      first_dt, last_dt, gen_at, total_ticks)

    HTML_OUT.parent.mkdir(parents=True, exist_ok=True)
    with open(HTML_OUT, 'w', encoding='utf-8') as f:
        f.write(html)

    log.info(f"  HTML written: {HTML_OUT.stat().st_size/1024:.0f} KB → {HTML_OUT}")

# ─────────────────────────────────────────────────────────────────────────────
#  HTML TEMPLATE
# ─────────────────────────────────────────────────────────────────────────────

def build_html(m25_js, m31_js, m25_n, m31_n, from_dt, to_dt, gen_at, total_ticks):
    return f"""<!DOCTYPE html>
<html lang="en">
<head>
<meta charset="UTF-8">
<meta name="viewport" content="width=device-width, initial-scale=1.0">
<title>#LTCUSD M25+M31 Renko Live</title>
...
:root{{--bg:#050810;--panel:#090d18;--sub:#0b1019;--b1:#131f30;--b2:#1a2f48;
  --bull:#26d97f;--bear:#e8365d;--acc:#38bdf8;--gold:#f5c842;--purple:#a78bfa;
  --muted:#3d5a72;--dim:#243648;--text:#8eafc4;--white:#ddeeff;}}
...
.topbar{{height:46px;background:rgba(5,8,16,.98);border-bottom:1px solid var(--b2);
...
.tright{{display:flex;align-items:center;gap:8px;font-size:.48rem;color:var(--muted);}}
...
.gentag{{font-size:.38rem;color:var(--dim);padding:2px 5px;border:1px solid var(--b1);border-radius:2px;}}
...
.infobar{{background:var(--sub);border-bottom:1px solid var(--b1);padding:3px 12px;
  display:flex;align-items:center;gap:10px;flex-shrink:0;flex-wrap:wrap;}}
.ib{{font-size:.42rem;color:var(--muted);}}
.ib b{{color:var(--bull);}}
.il{{display:flex;align-items:center;gap:4px;font-size:.42rem;color:var(--muted);}}
.sw{{width:20px;height:2px;border-radius:1px;}}
...
.ph{{height:36px;background:rgba(5,8,16,.95);border-bottom:1px solid var(--b2);
...
.ph-price{{font-size:.68rem;font-weight:700;}}
.ph-price.bull{{color:var(--bull)}}.ph-price.bear{{color:var(--bear)}}
.ph-dir{{font-size:.36rem;}}
.ph-dir.bull{{color:var(--bull)}}.ph-dir.bear{{color:var(--bear)}}
.ps{{display:flex;border-bottom:1px solid var(--b1);flex-shrink:0;}}
.psi{{flex:1;text-align:center;padding:2px 2px;border-right:1px solid var(--b1);}}
.psi:last-child{{border-right:none;}}
.psv{{display:block;font-size:.54rem;font-weight:700;color:var(--white);}}
.psv.bull{{color:var(--bull)}}.psv.bear{{color:var(--bear)}}.psv.gold{{color:var(--gold)}}.psv.acc{{color:var(--acc)}}
...
.pzb{{width:24px;height:24px;background:rgba(9,13,24,.92);border:1px solid var(--b2);
  color:var(--text);font-size:.75rem;cursor:pointer;display:flex;align-items:center;
  justify-content:center;border-radius:2px;}}
.pzb.w{{width:32px;font-size:.42rem;}}
.pif{{padding:2px 8px;border-top:1px solid var(--b1);background:var(--sub);
  display:flex;justify-content:space-between;font-size:.36rem;color:var(--dim);flex-shrink:0;}}
...
#tt{{position:fixed;background:rgba(8,11,20,.98);border:1px solid var(--b2);
  padding:7px 11px;font-size:.52rem;line-height:2;pointer-events:none;
  z-index:950;display:none;min-width:185px;box-shadow:0 6px 24px rgba(0,0,0,.8);}}
...
<div class="topbar">
  <div class="brand">
    ...
    <div>
      ...
      <div class="bsub">24-stage ÷3 cascade · {from_dt} → {to_dt} · {total_ticks:,} ticks saved · Keltner(200) · PPP2(200)</div>
    </div>
  </div>
  <div class="tright">
...
<div class="infobar">
  ...
  <span style="font-size:.42rem;color:var(--gold);">M25={m25_n:,} bricks · M31={m31_n:,} bricks · Rebuilds every 500 ticks</span>
</div>
<div class="charts-row">
  <div class="chart-panel">
    <div class="ph">
      <div><div class="ph-title ph-m25">TF25 — M25 · STAGE 19</div>
        <div class="ph-sub">BoxPts=0.01110 · Keltner(200) · PPP2(200)</div></div>
      <div style="text-align:right;">
        <div class="ph-price" id="p25-pr">—</div>
        <div class="ph-dir" id="p25-dr">—</div></div>
    </div>
    <div class="ps">
      <div class="psi"><span class="psv acc" id="25-n">{m25_n:,}</span><span class="psl">Bricks</span></div>
  ...
  <div class="chart-panel">
    <div class="ph">
      <div><div class="ph-title ph-m31">TF31 — M31 · STAGE 24</div>
        <div class="ph-sub">BoxPts=0.01127 · Keltner(200) · PPP2(200)</div></div>
      <div style="text-align:right;">
        <div class="ph-price" id="p31-pr">—</div>
        <div class="ph-dir" id="p31-dr">—</div></div>
    </div>
    <div class="ps">
      <div class="psi"><span class="psv acc" id="31-n">{m31_n:,}</span><span class="psl">Bricks</span></div>
    ...
    <div class="pif"><span>Stage 24 · fed by M25 bricks · repaint-free</span><span style="color:var(--bull)">✓ LOCKED</span></div>
...
let kelt25,ppp25,kelt31,ppp31;
let vs25=0,bW25=2,vs31=0,bW31=2;
let lastP=0,prevP=0,tickN=0;
...
function drawPanel(cv,ctx,wrap,B,k,p,vs,bW,acCol,sbhId,sbtId){{
  ...
  const PL=2,PR=60,PT=9,PB=16,CW=W-PL-PR,CH=H-PT-PB;
  ...
  if(k){{let go=false;ctx.beginPath();for(let i=0;i<N;i++){{const ki=off+i;if(k.U[ki]===null)continue;go?ctx.lineTo(XC(i),Y(k.U[ki])):(ctx.moveTo(XC(i),Y(k.U[ki])),go=true);}}for(let i=N-1;i>=0;i--){{const ki=off+i;if(k.L[ki]===null)continue;ctx.lineTo(XC(i),Y(k.L[ki]));}}ctx.closePath();ctx.fillStyle='rgba(255,255,255,.03)';ctx.fill();ctx.beginPath();go=false;ctx.strokeStyle='rgba(255,255,255,.48)';ctx.lineWidth=1;ctx.setLineDash([]);for(let i=0;i<N;i++){{const ki=off+i;if(k.U[ki]===null)continue;go?ctx.lineTo(XC(i),Y(k.U[ki])):(ctx.moveTo(XC(i),Y(k.U[ki])),go=true);}}ctx.stroke();ctx.beginPath();go=false;ctx.strokeStyle='rgba(255,255,255,.48)';ctx.lineWidth=1;for(let i=0;i<N;i++){{const ki=off+i;if(k.L[ki]===null)continue;go?ctx.lineTo(XC(i),Y(k.L[ki])):(ctx.moveTo(XC(i),Y(k.L[ki])),go=true);}}ctx.stroke();ctx.beginPath();go=false;ctx.strokeStyle='rgba(255,255,255,.26)';ctx.lineWidth=1;ctx.setLineDash([3,4]);for(let i=0;i<N;i++){{const ki=off+i;if(k.M[ki]===null)continue;go?ctx.lineTo(XC(i),Y(k.M[ki])):(ctx.moveTo(XC(i),Y(k.M[ki])),go=true);}}ctx.stroke();ctx.setLineDash([]);}}
  if(p){{let go=false;ctx.beginPath();go=false;ctx.strokeStyle='#00ff00';ctx.lineWidth=BW>=3?2:1;for(let i=0;i<N;i++){{const ki=off+i;if(p.up[ki]===null)continue;go?ctx.lineTo(XC(i),Y(p.up[ki])):(ctx.moveTo(XC(i),Y(p.up[ki])),go=true);}}ctx.stroke();ctx.beginPath();go=false;ctx.strokeStyle='#ff0000';ctx.lineWidth=BW>=3?2:1;for(let i=0;i<N;i++){{const ki=off+i;if(p.dn[ki]===null)continue;go?ctx.lineTo(XC(i),Y(p.dn[ki])):(ctx.moveTo(XC(i),Y(p.dn[ki])),go=true);}}ctx.stroke();ctx.beginPath();go=false;ctx.strokeStyle='rgba(0,255,255,.4)';ctx.lineWidth=1;for(let i=0;i<N;i++){{const ki=off+i;if(p.cb[ki]===null)continue;go?ctx.lineTo(XC(i),Y(p.cb[ki])):(ctx.moveTo(XC(i),Y(p.cb[ki])),go=true);}}ctx.stroke();}}
  show.forEach((b,i)=>{{const x=XL(i),y1=Y(b[1]),y2=Y(b[2]);const top=Math.min(y1,y2),bh=Math.max(1,Math.abs(y1-y2));if(b[4]===1){{const g=ctx.createLinearGradient(x,top,x,top+bh);g.addColorStop(0,'#26d97f');g.addColorStop(1,'#1aaa62bb');ctx.fillStyle=g;ctx.strokeStyle=BW>=2?'#26d97faa':'#26d97f33';}}else{{const g=ctx.createLinearGradient(x,top,x,top+bh);g.addColorStop(0,'#c02248aa');g.addColorStop(1,'#e8365d');ctx.fillStyle=g;ctx.strokeStyle=BW>=2?'#e8365daa':'#e8365d33';}}ctx.lineWidth=BW>=2?0.3:0.1;ctx.fillRect(x,top,BW,bh);if(BW>=2)ctx.strokeRect(x,top,BW,bh);}});
  const tg=ctx.createLinearGradient(0,0,W,0);tg.addColorStop(0,'transparent');tg.addColorStop(.2,acCol);tg.addColorStop(.8,acCol);tg.addColorStop(1,'transparent');ctx.strokeStyle=tg;ctx.lineWidth=1.5;ctx.setLineDash([]);ctx.beginPath();ctx.moveTo(PL,PT);ctx.lineTo(W-PR,PT);ctx.stroke();
  const lc=show[N-1][2],ly=Y(lc),lb=show[N-1][4]===1;ctx.strokeStyle=lb?'#26d97f55':'#e8365d55';ctx.lineWidth=1;ctx.setLineDash([2,4]);ctx.beginPath();ctx.moveTo(PL,ly);ctx.lineTo(W-PR,ly);ctx.stroke();ctx.setLineDash([]);ctx.fillStyle=lb?'#26d97f':'#e8365d';ctx.font='8px JetBrains Mono,monospace';ctx.textAlign='left';ctx.fillText(fmt(lc),W-PR+3,ly+3);
...
function drawBoth(){{
  drawPanel(cv25,ctx25,w25,B25,kelt25,ppp25,vs25,bW25,'#38bdf8','sbh25','sbt25');
  drawPanel(cv31,ctx31,w31,B31,kelt31,ppp31,vs31,bW31,'#f5c842','sbh31','sbt31');
}}
function updateStats(id,B,k,p){{
  ...
  set(id+'-n',n.toLocaleString(),'acc');set(id+'-bu',bull.toLocaleString(),'bull');
  set(id+'-be',bear.toLocaleString(),'bear');set(id+'-pc',((bull/n)*100).toFixed(1)+'%','gold');
  if(lKM!==null)set(id+'-km',fmt(lKM),'acc');
...
function connect(){{
  const dot=document.getElementById('cd'),txt=document.getElementById('ct');
  ...
  ws.onmessage=e=>{{
    try{{const msg=JSON.parse(e.data);if(msg.e!=='trade')return;
      const price=parseFloat(msg.p);tickN++;prevP=lastP;lastP=price;
      const pe=document.getElementById('lp');pe.textContent=price.toFixed(DIGITS);pe.className='pe '+(price>=prevP?'up':'dn');
      document.getElementById('tc').textContent=tickN.toLocaleString()+' ticks';
    }}catch(e){{}}
  }};
  ws.onerror=()=>{{dot.className='dot dead';}};
  ws.onclose=()=>{{dot.className='dot wait';setTimeout(connect,3000);}};
}}
function startRefresh(){{
  clearInterval(window._rl);progStart=Date.now();
  window._rl=setInterval(()=>{{
    const rem=Math.max(0,1-(Date.now()-progStart)/1000);
    const bar=document.getElementById('rp');if(bar)bar.style.width=(((1-rem)/1)*100)+'%';
    if(rem<=0){{drawBoth();progStart=Date.now();}}
  }},100);
}}
function setupSB(sbhId,sbtId,which){{
  let drag=false,x0=0,vs0=0;
  document.getElementById(sbhId).addEventListener('mousedown',e=>{{drag=true;x0=e.clientX;vs0=which===25?vs25:vs31;e.preventDefault();}});
  window.addEventListener('mousemove',e=>{{if(!drag)return;const tw=document.getElementById(sbtId).clientWidth;const B=which===25?B25:B31,bW=which===25?bW25:bW31,wrap=which===25?w25:w31;const vis=Math.max(1,Math.floor((wrap.clientWidth-58)/bW));const vs=Math.max(0,Math.min(B.length-vis,vs0+Math.round((e.clientX-x0)/tw*B.length)));if(which===25)vs25=vs;else vs31=vs;drawBoth();}});
  window.addEventListener('mouseup',()=>{{drag=false;}});
}}
setupSB('sbh25','sbt25',25);setupSB('sbh31','sbt31',31);
function setupTouch(cvEl,which){{
  let tx0=0,tvs0=0,lastDist=0;
  cvEl.addEventListener('touchstart',e=>{{tx0=e.touches[0].clientX;tvs0=which===25?vs25:vs31;if(e.touches.length===2)lastDist=Math.hypot(e.touches[0].clientX-e.touches[1].clientX,e.touches[0].clientY-e.touches[1].clientY);e.preventDefault();}},{{passive:false}});
  cvEl.addEventListener('touchmove',e=>{{if(e.touches.length===2){{const dist=Math.hypot(e.touches[0].clientX-e.touches[1].clientX,e.touches[0].clientY-e.touches[1].clientY);if(lastDist>0){{if(which===25)z25(bW25*(dist/lastDist));else z31(bW31*(dist/lastDist));}}lastDist=dist;}}else{{const bW=which===25?bW25:bW31,B=which===25?B25:B31,wrap=which===25?w25:w31;const vis=Math.max(1,Math.floor((wrap.clientWidth-58)/bW));const vs=Math.max(0,Math.min(B.length-vis,tvs0+Math.round((tx0-e.touches[0].clientX)/bW)));if(which===25)vs25=vs;else vs31=vs;drawBoth();}}e.preventDefault();}},{{passive:false}});
}}
...
function setupHover(cvEl){{
  cvEl.addEventListener('mousemove',e=>{{if(!cvEl._show)return;const rect=cvEl.getBoundingClientRect();const DPR2=window.devicePixelRatio||1;const mx=(e.clientX-rect.left)*(cvEl.width/rect.width/DPR2);const idx=Math.floor((mx-cvEl._PL)/(cvEl._CW/cvEl._N));if(idx<0||idx>=cvEl._N){{tt.style.display='none';return;}}const b=cvEl._show[idx],ki=cvEl._vS+idx;const dt=new Date(b[0]*1000).toISOString().replace('T',' ').slice(0,19)+' UTC';const bull=b[4]===1,k=cvEl._k,p=cvEl._p;document.getElementById('tth').innerHTML=(bull?'<span class="ttbull">▲ BULL</span>':'<span class="ttbear">▼ BEAR</span>')+' Brick #'+(ki+1)+' · '+dt;let ind='';if(k&&k.U[ki]!==null)ind+=`<span class="ttacc">Kelt: ${{fmt(k.U[ki])}} / ${{fmt(k.M[ki])}} / ${{fmt(k.L[ki])}}</span><br>`;if(p&&p.cb[ki]!==null){{const up=p.up[ki]!==null;ind+=`<span style="color:${{up?'#00ff00':'#ff0000'}}">PPP2: ${{fmt(p.cb[ki])}} ${{up?'▲':'▼'}}</span><br>`;}}document.getElementById('ttb').innerHTML=`Open: ${{fmt(b[1])}}<br>Close: ${{fmt(b[2])}}<br>Vol: ${{b[3].toLocaleString()}}<br>`+(ind?'<hr style="border-color:var(--dim);margin:3px 0">'+ind:'');tt.style.display='block';tt.style.left=(e.clientX+14)+'px';tt.style.top=Math.max(8,e.clientY-28)+'px';}});
  cvEl.addEventListener('mouseleave',()=>{{tt.style.display='none';}});
}}
setupHover(cv25);setupHover(cv31);
window.addEventListener('load',()=>{{
  resize();
  kelt25=keltner(B25,KELT_P);ppp25=ppp2(B25,PPP_P);
  kelt31=keltner(B31,KELT_P);ppp31=ppp2(B31,PPP_P);
  updateStats('25',B25,kelt25,ppp25);updateStats('31',B31,kelt31,ppp31);
  f25();f31();
  connect();startRefresh();
...
async def gap_fill(session, last_ts_sec: int):
    import aiohttp
    if last_ts_sec <= 0:
        return 0
    url = f"{BINANCE_REST}&startTime={last_ts_sec*1000}"
    try:
...
async def ws_loop():
    ...
    log.info("=" * 60)
    log.info("  LTCUSDT Cloud Feeder — Render.com")
    log.info(f"  Ticks file : {TICKS_FILE}")
    log.info(f"  HST file   : {HST_FILE}")
    log.info(f"  HTML out   : {HTML_OUT}")
    log.info(f"  Saved ticks: {total_ticks:,}")
    log.info(f"  Rebuild every {REBUILD_EVERY} ticks")
    log.info("=" * 60)

    # Do initial build if HST exists but HTML does not
    if HST_FILE.exists() and not HTML_OUT.exists():
        log.info("Initial chart build...")
        rebuild_chart()
        ...
        while True:
            try:
                ...
                async with websockets.connect(
                    ...
                    log.info("Connected — receiving live ticks")
                    ...
                    async for raw in ws:
                        try:
                            ...
                            if total_ticks % 1000 == 0:
                                ...
                                log.info(f"Tick #{total_ticks:,}  {dt.strftime('%H:%M:%S')} UTC  {price:.5f}")
